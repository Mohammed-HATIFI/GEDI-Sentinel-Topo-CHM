...
def train_antishrink_temporal_stage(
    *,
    model,
    train_dataset,
    val_dataset,
    ...
    warmstart_prediction_head_checkpoint: Path,
    run_dir: Path,
    device: str,
    seed: int = 42,
    batch_size: int = 1,
    ...
    """Isolated v4 warm-start run with GrowthLoss and anti-shrinkage.

    The frozen B4 parent and selected v4 prediction head are immutable
    provenance. Only the copied Conv3D prediction head is optimized and every
    output is written below the new run directory.
    """
    ...
    if "prediction_head" not in warmstart_state:
        raise KeyError(
            f"v4 warm-start checkpoint has no prediction_head: {warmstart_prediction_head_checkpoint}"
        )
    model.prediction_head.load_state_dict(warmstart_state["prediction_head"], strict=True)
    warmstart_copy = ckpt_dir / "warmstart_v4_best_READ_ONLY_COPY.ckpt"
    warmstart_provenance = safe_copy_reference(
        warmstart_prediction_head_checkpoint, warmstart_copy
    )
    (ckpt_dir / "warmstart_v4_best_meta.json").write_text(
        json.dumps(
            {
                "kind": "immutable v4 residual Conv3D prediction-head warm-start",
    ...
    config = {
        "protocol": "v4 warm-start -> Huber + official GrowthLoss + anti-shrinkage v5",
        "phase1_parent": phase1_provenance,
        "v4_prediction_head_warmstart": warmstart_provenance,
    ...
    print("\n" + "=" * 118)
    print("🚀 ISOLATED V5 FINE-TUNING — V4 + HUBER + GROWTHLOSS + ANTI-SHRINKAGE")
    print("=" * 118)
    print(f"📁 [RUN] {run_dir}")
    print(f"🧬 [PHASE-1 PARENT] {phase1_checkpoint}")
    print(f"🔥 [V4 WARM-START] {warmstart_prediction_head_checkpoint}")
    ...
    print(
        f"🧪 [LOSS] Huber(delta={huber_delta:g}) + {lambda_growth:g} x official GrowthLoss "
    ...
    elapsed = float(time.time() - start_time)
        ...
        "v4_warmstart_sha256": warmstart_provenance["source_sha256"],
    }
